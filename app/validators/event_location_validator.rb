...
private
  def validate_postal_code event
    if event.in_usa?
      unless event.postal_code =~ /^\d{5}(-\d{4})?$/
        event.errors.add :postal_code, "is not a valid U.S. postal code"
      end
    elsif event.in_canada?
      unless event.postal_code =~ /^\D\d\D((-| )?\d\D\d)?$/
        event.errors.add :postal_code, "is not a valid Canadian postal code"
      end
    end      
  end

  def validate_city event
    event.errors.add "City is blank" unless event.city
  end

  def validate_country_code event
    event.errors.add "Country is blank" unless event.country_code 
  end
  ...
  def validate_state event
    if event.in_usa?
      ...
      if event.state.blank? or not valid_us_states.include?(event.state)
        event.errors.add :state, "is not a valid U.S. state"
      end      
    elsif event.in_canada?
      unless event.state_is_canadian_province?
        event.errors.add :state, "is not a valid Canadian province"
      end
    end
  end
  ...
  def validates_mappable event
    ...
    if (event.in_usa? || event.in_canada?) && !(event.latitude && event.longitude)
      event.errors.add_to_base "Not enough information provided to place event on a map. Please give us at minimum a valid postal code."
    end
  end
