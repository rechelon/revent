class EventSweeper < ActionController::Caching::Sweeper
  ...
  def after_create(event)
    ...
    Rails.logger.info("Expired caches for new event")
  end
  ...
  def after_update(event)
    ...
    Rails.logger.info("Expired caches for updated event #{event.id}")
  end

  def after_destroy(event)
    expire_event_pages_and_fragments(event)
    expire_event_list_pages(event)
    Rails.logger.info("Expired caches for deleted event #{event.id}")
  end
